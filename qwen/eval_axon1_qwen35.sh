...
BEST=$(ls -td runs/mids_head/mids_v1_* | head -1)/best.pth
...
for g in 0 1 2 3; do
  CUDA_VISIBLE_DEVICES=$g $VPY qwen/gen_score_axon1.py gen \
    ...
    --out "$OUTD/qwen35_4b_axon1_$g.json" --which-part $g --n-divided 4 \
    > "$OUTD/gen35_$g.log" 2>&1 &
  pids+=($!)
done
fail=0; for i in "${!pids[@]}"; do wait "${pids[$i]}" || { echo "gen shard $i FAILED (see $OUTD/gen35_$i.log)"; fail=1; }; done
...
for g in 0 1 2 3; do
  CUDA_VISIBLE_DEVICES=$g $GPY qwen/gen_score_axon1.py score \
    --answers "$OUTD/qwen35_4b_axon1_$g.json" --mids "$BEST" \
    --out "$OUTD/qwen35_4b_axon1_$g.txt" --device cuda:0 \
    > "$OUTD/score35_$g.log" 2>&1 &
  pids+=($!)
done
fail=0; for i in "${!pids[@]}"; do wait "${pids[$i]}" || { echo "score shard $i FAILED"; fail=1; }; done
[ "$fail" = 1 ] && { echo "SCORE FAILED"; exit 1; }

echo "==== merge shards -> qwen35_4b_axon1.txt ===="
{ echo "# columns: OK truth pred fake_score match_score frame_key";
  for g in 0 1 2 3; do grep -v '^#' "$OUTD/qwen35_4b_axon1_$g.txt"; done; } > "$OUTD/qwen35_4b_axon1.txt"
echo "  merged $(grep -vc '^#' "$OUTD/qwen35_4b_axon1.txt") lines"
