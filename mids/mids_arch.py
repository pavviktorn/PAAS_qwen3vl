...
class MIDS(nn.Module):
    def __init__(self, dim=768, **kwargs):
        ...
        # PAAS: default to the bundled encoders, resolved relative to the project root
        # (this file lives in <root>/ffaa/mids/); $PAAS_T5/$PAAS_CLIP override.
        import os as _os
        _proot = _os.path.dirname(_os.path.dirname(_os.path.dirname(_os.path.abspath(__file__))))
        _t5 = _os.environ.get("PAAS_T5", _os.path.join(_proot, "base_models", "t5-base"))
        _clip = _os.environ.get("PAAS_CLIP", _os.path.join(_proot, "base_models", "clip-vit-large-patch14-336"))
